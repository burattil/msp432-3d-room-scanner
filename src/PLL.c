...
// bus frequency is 56 MHz/(PSYSDIV+1) = 56 MHz/(3+1) = 14 MHz
...
void PLL_Init(void){ uint32_t timeout;
  ...
  //    frequency of 56 MHz with MFRAC=0 to reduce jitter.  To run at a frequency
  //    that is not an integer divisor of 480 MHz, change this section.
  //    fVC0 = (fXTAL/(Q + 1)/(N + 1))*(MINT + (MFRAC/1,024))
  //    fVCO = 56,000,000 Hz (arbitrary, but presumably as small as needed)
#define FXTAL 25000000  // fixed, this crystal is soldered to the Connected Launchpad
#define Q            0
#define N           24  // chosen for reference frequency within 4 to 30 MHz
#define MINT        56  // 56,000,000 = (25,000,000/(0 + 1)/(24 + 1))*(56 + (0/1,024))
