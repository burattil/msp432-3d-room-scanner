...
// the system clock to 14 MHz.  This matters for the function
...
void SysTick_Wait10ms(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
    SysTick_Wait(140000);  // wait 10ms (assumes 14 MHz clock)
  }
}
...
void SysTick_Wait10us(uint32_t delay){
  uint32_t i;
  for(i=0; i<delay; i++){
    SysTick_Wait(140);  // wait 10ms (assumes 14 MHz clock)
  }
}
